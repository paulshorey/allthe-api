		...
		*/
		global.m.ACCOUNT.collection('admin').insertMany([userData], function(error, data) {
			if (error) {
				...
				/***
				* CREATE AGGREGATOR COLLECTION
				*/
				global.m.AGGREGATOR.createCollection(account_id+'', (error , collection) => {
					if (error) {
						global.http_response(res, 403, "access error: mongoDB createCollection: " + error ); return;
					} else {
